...
**Launcher**:
The double-clickable app — macOS and Windows — that is the Sandbox User's entire interface to Agentbox. Trusted, host-side code (not Claude) that hides Docker: it starts the Box, shows the Project home screen, drops the user into Claude Code, and brokers file Uploads.
...
**GitHub Account**:
The Sandbox User's own GitHub account, connected once through the Launcher by OAuth device flow — they type a code at github.com, never a token. The resulting token is held by the Launcher on the host, encrypted with the OS keystore, and is the only credential Agentbox has ever had. It never enters the Box. It carries `repo` scope, so it reads and writes every private repository that user owns: the containment in Save to GitHub is what stands between that and a container running Claude with permissions bypassed (ADR 0006).
...
_Avoid_: sync, backup (it is neither continuous nor automatic), "git integration" (Agentbox configures no remote inside the Box — it only reads the one an Import brought — the Box never holds the token, and there is no clone-in)
...
**Refresh on Launch**:
On every start, the Launcher pulls the latest Box definition from the public Agentbox GitHub repo and rebuilds the image if it changed, then runs `claude update` inside the running Box (the image's npm layer is cached, so a rebuild alone would keep shipping a stale Claude). This is the sole update mechanism, and the same one runs on demand from **Update Agentbox** on the home screen — for the Sandbox User who never quits the Launcher and would otherwise sit on last week's Box until they did. On demand it also recreates the container, because a rebuilt image changes nothing while the old one is still running; that ends every open Claude session, so it is confirmed first. Requires no credentials because the repo is public. Both halves are best-effort: an offline laptop still opens on its last-built image and its baked Claude, and is told so rather than shown a silent "up to date".

**Database**:
A PostgreSQL server in its own container (`agentbox-postgres`) beside the Box, started with it on every launch. It lives on an **internal** docker network (`agentbox-db`, pinned subnet 172.30.0.0/24): internal means docker gives it no route out at all, so the Database cannot escape that network, and no port is published, so nothing on the host or LAN can reach it either. The Box is attached to the network as a second interface, and the Egress Policy opens exactly one hole — TCP 5432 to the pinned subnet — so a Project reaches it at `postgresql://postgres:postgres@agentbox-postgres:5432/postgres` and nothing else on the subnet. The fixed password is a non-secret by construction: the socket is unreachable from anywhere but inside the Box, and it guards nothing but this disposable data. Data persists on a named volume, like the Workspace.
